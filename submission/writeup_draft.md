...
**Native function calling:** We leverage Gemma 4's built-in tool-use training. Our six tools are defined in OpenAI-compatible JSON Schema format and embedded in the system prompt. The model outputs structured `<tool_call>` JSON when it determines a lookup is needed.
